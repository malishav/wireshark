 ...
 */
static int
dissect_coap_options_main(tvbuff_t *tvb, packet_info *pinfo, proto_tree *coap_tree, gint offset, guint8 opt_count, guint *opt_num, gint coap_length, coap_info *coinfo, coap_common_dissect_t *dissect_hf)
{
	guint8      opt_jump;
 ...
 */
int
dissect_coap_options(tvbuff_t *tvb, packet_info *pinfo, proto_tree *coap_tree, gint offset, gint coap_length, coap_info *coinfo, coap_common_dissect_t *dissect_hf)
{
	guint  opt_num = 0;
	int    i;
	guint8 endmarker;
	...
	for (i = 1; offset < coap_length; i++) {
		offset = dissect_coap_options_main(tvb, pinfo, coap_tree,
		    offset, i, &opt_num, coap_length, coinfo, dissect_hf);
		if (offset == -1)
			return -1;
...
void
dissect_coap_payload(tvbuff_t *tvb, packet_info *pinfo, proto_tree *coap_tree, proto_tree *parent_tree, gint offset, gint length, guint8 code_class, coap_info *coinfo, coap_common_dissect_t *dissect_hf, gboolean oscore)
{
	proto_tree *payload_tree;
	proto_item *payload_item, *length_item;
	tvbuff_t   *payload_tvb;
	guint	    payload_length = length - offset;
...
dissect_coap(tvbuff_t *tvb, packet_info *pinfo, proto_tree *parent_tree, void* data _U_)
{
	...
	coinfo->oscore_info = (void *) wmem_new0(wmem_packet_scope(), oscore_info_t);
			...
			if (!coap_trans) {
				if ((!PINFO_FD_VISITED(pinfo)) && (code_class == 0)) {
					...
					if (coinfo->oscore_info) {
						coap_trans->oscore_info = wmem_memdup(wmem_file_scope(), coinfo->oscore_info, sizeof(oscore_info_t));
				...
				if ((code_class >= 2) && (code_class <= 5)) {
					...
					if (coap_trans->oscore_info) {
						...
						coinfo->oscore_info = wmem_memdup(wmem_packet_scope(), coap_trans->oscore_info, sizeof(oscore_info_t));
						coinfo->oscore_info->kid = (guint8 *) wmem_memdup(wmem_packet_scope(), coap_trans->oscore_info->kid, coap_trans->oscore_info->kid_len);
						coinfo->oscore_info->kid_context = (guint8 *) wmem_memdup(wmem_packet_scope(), coap_trans->oscore_info->kid, coap_trans->oscore_info->kid_context_len);
